...
def evaluate_split(dataset: Dataset, 
    ...
    for i, sample in enumerate(tqdm(eval_dataset, desc=f"Transcribing speech.")):
        ...
        print(f"Language: {language} -- Sample: {i}")
        print(f"Prediction: {pred_text}")
        print(f"Reference 1: {ref_text_1}")
        print(f"Reference 2: {ref_text_2}")
        print(f"Reference 3: {ref_text_3}")
        """
        Language: wolaytta -- Sample: 0
        Prediction: isi isi heran asay palaba pokoba giyoba uyes mehiya suta qanxidi goraobare gogiyaga ba bola silo malatin uyishin haga watidi mitika gays tana tumu qopin shene yes hage
        Reference 1: issi issi heeran asay palaba pokkoba giyobaa uyeesmehiya suuttaa qanxxidi goora''o bare goggiyagaa ba bollaassi loo malatin uyiishin hagaa waatidi mittiikaa gaystana tumu qoppin sheneyees hagee
        Reference 2: isi isi heran asay palaba pokoba giyoba uyesmehiya suta qanxidi gora'o bare gogiyaga ba bolasi lo malatin uyishin haga watidi mitika gaystana tumu qopin sheneyes hage
...
def main():
    ...
    LANGUAGES = ['wolaytta', 'afaan_oromo', 'sidama',  'tigrinya',  'amharic']
    ...
    for lang in LANGUAGES:
        ...
        print(f"Processing samples for {lang}...")

        eval_dataset = dataset.filter(lambda x: x['language'] == lang)

        print(f"Number of samples for {lang}: {len(eval_dataset)}")

        results = {}

        results[lang] = evaluate_split(
            eval_dataset, model, processor, device, language=lang, text_column=text_column
        )
