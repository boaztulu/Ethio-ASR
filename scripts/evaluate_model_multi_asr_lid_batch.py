...
def evaluate_split(dataset: Dataset,
    ...
    num_samples = len(eval_dataset) # 32 # len(eval_dataset)
    ...
    for batch_start in tqdm(range(0, len(eval_dataset), batch_size),
        ...
        for s, (pred_lid, pred_transcript) in zip(batch, results):
            ...
            pred_lid_tokens.append(lang_mapping_dict.get(final_pred_lid, lang_mapping_dict['[NO_LID_TOKEN]']))
    ...
    lid_accuracy = lid_accuracy_metric.compute(predictions=pred_lid_tokens, references=true_lid_tokens)
    ...
    for lang in lang_preds:
        ...
        if not lp:
            continue
        l_wer = wer_metric.compute(predictions=lp, references=lr)
        l_cer = cer_metric.compute(predictions=lp, references=lr)
        per_lang_metrics[lang] = {'wer': l_wer, 'cer': l_cer, 'samples': len(lp)}
