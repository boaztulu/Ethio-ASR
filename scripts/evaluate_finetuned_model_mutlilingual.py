...
def transcribe_audio(audio_array: Audio, 
    ...
    predicted_ids = predicted_ids[predicted_ids != char_to_id['[PAD]']]  # [PAD] token

    predicted_sequence = [ 
        id_to_char[token_id.item()] for token_id in predicted_ids
    ]

    # replace pipe | with a space 
    predicted_sequence = [
        char if char != '|' else ' ' for char in predicted_sequence
    ]
    ...
    for token_id in predicted_ids:
        ...
        if token_id != char_to_id['|'] and token_id != prev_id:
