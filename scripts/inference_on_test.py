...
def run_inference(dataset_path: str, 
    ...
    logging.info(f"Starting inference on a specific {split} split...")
    ...
    # logging.info first sample for debugging
    logging.info(f"First sample audio ID: {test_dataset[0]}")
    
    for i, sample in enumerate(tqdm(test_dataset, desc="Transcribing")):
